...
Here is an example of what the metadata could look like if you wanted to put the `README.md` file into a subfolder called `projectdocs`.

```{r, eval = FALSE}
readme <- 
  list(
    template_filename = system.file("project_templates/default_readme.md", package = "starter"), 
    filename = "projectdocs/README.md", 
    glue = TRUE
  )
```
